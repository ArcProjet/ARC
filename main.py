...
def solveGrid(grid,name):
    grillestrain, grillestest = grid
    
    if len(grillestrain) == 2:
        gridInput = grillestrain[0]['input'], grillestrain[1]['input']
        gridWishes = grillestrain[0]['output'], grillestrain[1]['output']
    ...
    print("[+] Best Individu functions before optimization")

    print(bestIndividu.fonctions)

    #bestIndividu.fonctions = fonctions

    print("[+] Best Individu functions after optimization")
    ...
    write += str(g.comparer(grillestrain[0]['output'])) + ','
    ...
    alldata.append(g.data)
    write += str(g.comparer(grillestrain[1]['output'])) + ','
    ...
    if len(grillestrain) > 2:
        ...
        alldata.append(g.data)
        write += str(g.comparer(grillestrain[2]['output'])) + ','
    ...
    alldata.append(g.data)
    write += str(g.comparer(grillestest[1]))

    # displayGrid(grillestest[0],grillestest[1],g.data,"Grille de départ","Grille de Fin","Grille obtenu")
    #displayGrid([grillestrain[0]['input'], grillestrain[1]['input'], grillestrain[2]['input'], grillestest[0]],
    #            [grillestrain[0]['output'], grillestrain[1]['output'], grillestrain[2]['output'], grillestest[1]],
    #            alldata, "Départ", "Fin", "Obtenu")
